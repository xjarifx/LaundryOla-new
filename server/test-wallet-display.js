  ...
  try {
    ...
    if (
      ...
      transactionsResponse.data.data.slice(0, 5).forEach((tx, index) => {
        ...
        console.log(
          `  - Expected Display: ${tx.type === "CREDIT" ? "+" : "-"}₹${Math.abs(
      ...
      const addMoneyTransactions = transactionsResponse.data.data.filter(
        (tx) => tx.type === "CREDIT"
      );
      console.log(
        `✅ Found ${addMoneyTransactions.length} CREDIT transactions (Add Money)`
      );

      if (addMoneyTransactions.length > 0) {
        const sampleCredit = addMoneyTransactions[0];
        const displayAmount = Math.abs(parseFloat(sampleCredit.amount)).toFixed(
          2
        );
        console.log(
          `✅ Sample Add Money transaction will display as: +₹${displayAmount}`
        );

        if (parseFloat(sampleCredit.amount) < 0) {
          console.log(
            "⚠️ Warning: Add Money transaction has negative amount in database"
          );
        } else {
          console.log(
            "✅ Add Money transaction has positive amount in database"
          );
        }
      }
    } else {
